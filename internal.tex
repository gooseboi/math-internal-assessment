...
$$
D^{-n}f = \frac{1}{n!}\int_a^{t_1}\left(t_1-t\right)^nf(t)\di t
$$
...
$\frac{1}{n!}$. Generalising factorials to all $\mathbb{R}$ is complex, involving
techniques in real analysis. However, since this is not the focus of this exploration,
I will use the result without proving it. This generalisation is named $\Gamma(x)$, and its formula is given as
$$
\Gamma(x)=\int_0^{\infty} e^{-t}t^{x-1}dt = (x-1)!
$$
Having this function, we can thus express the formula for repeated integration as
$$
D^{-\alpha}f = \frac{1}{\Gamma\left(\alpha+1\right)}\int_a^{t_1}\left(t_1-t\right)^\alpha f(t)\di t
$$

Having this, we can now define fractional calculus.
...
now define an operator that can ``take the derivative $\alpha$ times'', where
$\alpha$ could be any real number
